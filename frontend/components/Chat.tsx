...
export function Chat({ messages, onSendMessage, isLoading }: ChatProps) {
  ...
  // Watch for special business selector trigger message (anywhere in list) and only if business unit has not been selected yet
  useEffect(() => {
    const hasTrigger = messages.some(
      (m) => m.role === "assistant" && m.content === "DISPLAY_BUSINESS_SELECTOR"
    );
    // Show selector if trigger exists and business unit not chosen yet
    if (hasTrigger && !selectedBusinessUnit) {
      setShowBusinessSelector(true);
    }
  }, [messages, selectedBusinessUnit]);
  ...
  const handleBusinessUnitSelect = useCallback(
    (unit: "promoselect" | "suitup") => {
      setSelectedBusinessUnit(unit);
      setShowBusinessSelector(false);
      onSendMessage(unit === "promoselect" ? "Promoselect" : "SuitUp");
    },
    [onSendMessage]
  );
    ...
    <div className="flex flex-col h-full flex-1 bg-white shadow-sm border border-gray-200 border-t-0 rounded-xl">
      <div className="bg-blue-600 text-white h-12 px-4 flex items-center rounded-t-xl">
        <h2 className="font-semibold text-sm sm:text-base lg:text-lg">
          Chat de Productos Promocionales
        </h2>
      </div>
      {/* Messages */}
      <div className="flex-1 overflow-y-auto min-h-0 md:px-4 pt-4 pb-20">
        {messages.map((msg, idx) => {
          if (msg.content === "DISPLAY_BUSINESS_SELECTOR") return null; // Skip rendering marker message
          return (
            <div
              key={idx}
